...
public class leagueScraperRunner {
	...
	//Method containing case switch to skip downloads based off number of preseason weeks
	public void caseDownload(int numPreWeeks, int boxScoreStart, int loopValue, 
			String leagueURL, String downloadToFile) throws IOException {
				//joptionpane showing the user that the download has started
				leagueScraperRunner.infoBox("Download has started successfully", "Download Success Message");
				...
				//joptionpane showing the user that the download has started
				leagueScraperRunner.infoBox("Download has finished successfully", "Download Success Message");
				
	}

	public static void main(String[] args) throws IOException, InterruptedException {
		leagueScraperRunner temp = new leagueScraperRunner();
		//Making our swing window thread safe and starting it
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				main = new Window(temp);
				main.show();
			}
		});
		 ...
		 * // wanted info // Loading our txt file File textFile = new
		 * File("strings.txt"); // Opening a scanner to read from the textFile Scanner
		 * scnr = null; try { scnr = new Scanner(textFile); } catch
		 * (FileNotFoundException e) { // TODO Auto-generated catch block
		 * System.out.println("Error opening strings.txt"); System.exit(1); } // Scanner
		 * opened the file and now we need to store our variables String downloadToFile
		 * = scnr.nextLine();// first line is where to download logs to String leagueURL
		 * = scnr.nextLine();// second line is the league to scrape int boxScoreStart =
		 * Integer.parseInt(scnr.nextLine());// third line is the box score starting int
		 * loopValue = Integer.parseInt(scnr.nextLine());// fourth line is the number of
		 * seasons int numPreWeeks = Integer.parseInt(scnr.nextLine());// fifth line is
		 * num of preseason weeks
		 */		
		/*
		 ...
		 * numPreWeeks);
		 */
		//caseDownload(numPreWeeks, boxScoreStart, loopValue, leagueURL, downloadToFile);
